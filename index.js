...
function parseDataElements (dataElements, dataElementEncoding) {
  ...
  for (var i = 1; i < dataElements.length; i++) {
    ...
    if (_.has(dataElementEncoding, dataElementId)) {
      if (_.has(dataElementEncoding[dataElementId], 'name')) { dataElement.name = dataElementEncoding[dataElementId].name } else { dataElement.name = dataElementEncoding[dataElementId] }
    }
...
function parseDataElementComponents (dataElementComponents, dataElementComponentEncoding) {
  ...
  for (var i = 0; i < dataElementComponents.length; i++) {
    ...
    if (_.has(dataElementComponentEncoding, dataElementComponentId)) { dataElementComponent.name = dataElementComponentEncoding[dataElementComponentId] }
